...
func (f *file) Open(ctx context.Context, req *fuse.OpenRequest, resp *fuse.OpenResponse) (fs.Handle, error) {
	...
	log.Infof("FUSE: Opening[jid=%v] %v", activity.GetID(ctx), f)
	if plugin.ReadAction.IsSupportedOn(f.entry) {
		...
		log.Infof("FUSE: Opened[jid=%v] %v", activity.GetID(ctx), f)
		activity.Record(ctx, "FUSE: Opened %v", f)
		return &fileHandle{r: content, id: f.String()}, nil
	}
	log.Warnf("FUSE: Error[Open,%v,jid=%v]: cannot open this entry", f, activity.GetID(ctx))
...
func (fh fileHandle) Release(ctx context.Context, req *fuse.ReleaseRequest) error {
	log.Infof("FUSE: Release[jid=%v] %v", activity.GetID(ctx), fh.id)
...
func (fh fileHandle) Read(ctx context.Context, req *fuse.ReadRequest, resp *fuse.ReadResponse) error {
	...
	log.Infof("FUSE: Read[jid=%v] %v, %v/%v bytes starting at %v: %v", activity.GetID(ctx), fh.id, n, req.Size, req.Offset, err)
